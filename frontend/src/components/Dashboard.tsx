...
interface DashboardProps {
  ...
  onSendMessage: (threadId: string, message: string, filters?: FilterState) => void;
...
export function Dashboard({ 
  ...
  const handleApplyFilters = (filters: FilterState) => {
    ...
    if (activeThread) {
      ...
      const filterSummary = [];
      if (filters.organism.length > 0) filterSummary.push(`Organism: ${filters.organism.join(', ')}`);
      if (filters.exposureType.length > 0) filterSummary.push(`Exposure: ${filters.exposureType.join(', ')}`);
      if (filters.tissueSystem.length > 0) filterSummary.push(`System: ${filters.tissueSystem.join(', ')}`);
      if (filters.duration.length > 0) filterSummary.push(`Duration: ${filters.duration.join(', ')}`);
      if (filters.studyType.length > 0) filterSummary.push(`Study Type: ${filters.studyType.join(', ')}`);
      if (filters.missionContext.length > 0) filterSummary.push(`Mission: ${filters.missionContext.join(', ')}`);
      
      if (filterSummary.length > 0) {
        onSendMessage(activeThread.id, `Apply these research filters: ${filterSummary.join(' • ')}`, filters);
      }
    }
  };
        ...
        {activeThread ? (
          ...
          <div className="flex-1 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
            <div className="text-center">
              ...
              <h3 className="text-xl text-emerald-100 mb-2">No conversation selected</h3>
              <p className="text-emerald-300/70 mb-6 max-w-md">Choose a thread from the sidebar or start exploring the frontier of space biology research</p>
              <button
